...
const TRANSLATIONS = {
  fa: {
    ...
    tab_options: "تنظیمات خروجی",
    optTitle: "پیکربندی سورس Flet",
    optDesc: "شخصی‌سازی کدهای ساخته شده برای فایل پایتون:",
    optThemeAccent: "رنگ قالب برنامه‌نویسی Flet:",
    optAutoInstall: "نصب خودکار کتابخانه‌های غایب",
    optAutoInstallDesc: "افزودن ماژول دانلودر pip جهت دانلود و اجرای بی دردسر برنامه بدون خطای ماژول پایتون",
    optLang: "زبان اولین اجرای اپلیکیشن:",
    optSavedToast: "پارامترهای بهینه‌سازی کدهای فلت با موفقیت روی سورس اعمال شدند! 🛠️"
  },
  en: {
    ...
    tab_options: "Flet Options",
    optTitle: "Asynchronous Flet Parameters",
    optDesc: "Optimize and customize the generated main.py script:",
    optThemeAccent: "Primary GUI Seed Theme:",
    optAutoInstall: "Auto-install Missing Packages",
    optAutoInstallDesc: "Inject dynamic loaders so computers lacking flet/httpx automatically download them via pip at runtime.",
    optLang: "Startup Program Language:",
    optSavedToast: "Flet code custom parameters compiled successfully! 🛠️"
  }
};
...
export default function App() {
  ...
  // Navigation State
  const [activeTab, setActiveTab] = useState<"deploy" | "assets" | "options">("deploy");

  // Flet Customization Options States
  const [fletThemeAccent, setFletThemeAccent] = useState<"indigo" | "emerald" | "amber" | "crimson" | "teal">("indigo");
  const [fletSelfInstall, setFletSelfInstall] = useState<boolean>(true);
  const [fletDefaultLanguage, setFletDefaultLanguage] = useState<"fa" | "en">("fa");

  const getCustomizedFletCode = () => {
    let code = fletPythonCode;

    // 1. Dependency Auto-Installer loader toggle
    if (!fletSelfInstall) {
      const idx = code.indexOf("import asyncio");
      if (idx !== -1) {
        code = code.substring(idx);
      }
    }

    // 2. Flet Default Start Language
    if (fletDefaultLanguage !== "fa") {
      code = code.replace('self.lang = "fa"', `self.lang = "${fletDefaultLanguage}"`);
    }

    // 3. Theme seed color schemes replacement
    if (fletThemeAccent !== "indigo") {
      const upper = fletThemeAccent.toUpperCase();
      code = code.replaceAll("ft.colors.INDIGO", `ft.colors.${upper}`);
      code = code.replaceAll("ft.colors.INDIGO_300", `ft.colors.${upper}_300`);
      code = code.replaceAll("ft.colors.INDIGO_100", `ft.colors.${upper}_100`);
      code = code.replaceAll("ft.colors.INDIGO_900", `ft.colors.${upper}_900`);
    }
  ...
  // Abort controller ref for pipeline cancellation
  ...
  const handleCopyFletCode = () => {
    navigator.clipboard.writeText(getCustomizedFletCode());
    setCopiedFlet(true);
  ...
  const buildReportString = (data: DeploymentAsset) => {
    return (
      `🚀 **${lang === "fa" ? "استقرار پنل اوبسیدین با موفقیت انجام شد!" : "Obsidian Panel Deployed Successfully!"}** 🚀\n` +
      `-----------------------------------------\n` +
      `🌐 **${lang === "fa" ? "آدرس پنل" : "Panel Address"}:**\n` +
      `\`${data.url}\`\n\n` +
      `📧 **${lang === "fa" ? "ایمیل کلودفلر" : "Cloudflare Email"}:**\n` +
      `\`${data.email}\`\n\n` +
      `🔑 **${lang === "fa" ? "رمز ادمین پنل" : "Admin Password"}:**\n` +
      `\`${data.admin_pass}\`\n\n` +
      `🛡️ **${lang === "fa" ? "کلید عمومی کلودفلر" : "Global API Key"}:**\n` +
      `\`${data.global_key || globalKey || '-'}\`\n\n` +
      `🆔 **${lang === "fa" ? "شناسه حساب" : "Account ID"}:**\n` +
      `\`${data.account_id || accountId || '-'}\`\n\n` +
      `📦 **${lang === "fa" ? "نیم‌اسپیس KV" : "KV Namespace"}:**\n` +
      `\`${data.kv}\`\n\n` +
      `📂 **${lang === "fa" ? "پایگاه داده ریلیشنال D1" : "D1 Relational DB"}:**\n` +
      `\`${data.db}\`\n\n` +
      `⏱️ **${lang === "fa" ? "تاریخ و زمان استقرار" : "Deployment Timestamp"}:**\n` +
      `\`${data.date}\`\n` +
      `-----------------------------------------\n` +
      `💻 Developed by Mohammadsadegh Ghasemi\n` +
  ...
  const handleCancel = () => {
    ...
    setLogs((prev) => [...prev, { message: "Deployment pipeline cancelled by user.", status: "warn" }]);
    setLiveStatus("Cancelled");
    triggerToast(lang === "fa" ? "فرایند توسط کاربر لغو شد" : "Deployment cancelled by user.", "warn");
  };
  ...
  const handleDeploy = async (e: React.FormEvent) => {
    ...
    setLogs([]);
    setLiveStatus("Starting Obsidian bundle retrieval...");
    ...
    try {
      ...
      addLog("Stage 1/2: Pulling Obsidian raw package bundle from GitHub...", "info");
      const codeResp = await fetch("/api/fetch-github-code", { signal: controller.signal });
      if (!codeResp.ok) {
        throw new Error(`Failed to download worker.js source from GitHub repository (status: ${codeResp.status}).`);
      }
      const codeData = await codeResp.json();
      if (!codeData.success) {
        throw new Error(codeData.error || "GitHub repository resource fetching failed.");
      }
      setProgressPercent(15);
      addLog("Latest worker bundle verified and loaded securely!", "success");

      // 2. Begin server-side structured streaming pipeline setup
      addLog("Stage 2/2: Initializing active deployment orchestration pipeline on Cloudflare...", "info");
      setLiveStatus("Contacting deployment API...");
      ...
      if (!deployResp.body) {
        throw new Error("Unable to establish readable streaming pipeline on server.");
      }
        ...
        for (const line of lines) {
          if (line.startsWith("data: ")) {
            try {
              ...
              // Update progress bar matching the step prefix
              ...
              // Stop the build immediately on crash/failed server status
              if (status === "error") {
                setLiveStatus("Deployment pipeline crashed.");
                triggerToast(lang === "fa" ? "فرایند با خطا مواجه شد و متوقف گردید" : "Deployment pipeline failed.", "error");
                setIsDeploying(false);
                abortControllerRef.current = null;
                return;
              }
      ...
      if (error.name === "AbortError" || error.message?.includes("aborted")) {
        // Aborted purposefully by user, logs already added via handleCancel
        return;
      }
      addLog(`Deployment pipeline failed: ${error.message}`, "error");
          ...
          <div className={`w-full max-w-2xl rounded-3xl p-6 md:p-8 border shadow-2xl ${
            ...
            <div className="mt-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {/* Live Worker URL */}
                <div className="p-3 bg-indigo-500/5 rounded-2xl border border-indigo-500/10 flex items-center justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <span className="text-[10px] text-slate-400 block uppercase font-bold">{lang === "fa" ? "آدرس نهایی ورکر فعال" : "Live Worker URL"}</span>
                    <a href={reportData.url} target="_blank" rel="noreferrer" className="text-sm text-indigo-400 font-bold hover:underline break-all flex items-center gap-1.5 mt-1 leading-tight">
                      {reportData.url}
                  ...
                  <button 
                    onClick={() => {
                      ...
                      triggerToast(lang === "fa" ? "آدرس ورکر کپی شد" : "Live Worker URL copied", "success");
                    }}
                    className="p-2 bg-slate-500/10 hover:bg-indigo-500/20 rounded-xl text-slate-350 hover:text-indigo-400 transition-all cursor-pointer shrink-0"
                    title="Copy Link"
                  >
                ...
                {/* Panel Admin Secret */}
                <div className="p-3 bg-red-500/5 rounded-2xl border border-red-500/10 flex items-center justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <span className="text-[10px] text-slate-400 block uppercase font-bold">{lang === "fa" ? "کلمه عبور وب‌پنل ادمین" : "Panel Admin Secret"}</span>
                    <span className="text-sm text-red-400 font-mono font-bold block mt-1 break-all leading-tight">{reportData.admin_pass}</span>
                  </div>
                  <button 
                    onClick={() => {
                      navigator.clipboard.writeText(reportData.admin_pass);
                      triggerToast(lang === "fa" ? "پسورد پنل کپی شد" : "Panel password copied", "success");
                    }}
                    className="p-2 bg-slate-500/10 hover:bg-red-500/20 rounded-xl text-slate-350 hover:text-red-400 transition-all cursor-pointer shrink-0"
                ...
                <div className="p-3 bg-slate-500/5 rounded-2xl border border-slate-500/10 flex items-center justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <span className="text-[10px] text-slate-400 block uppercase font-bold">{t.emailLbl}</span>
                    <span className="text-xs font-mono font-medium block mt-1 break-all leading-tight text-indigo-300">{reportData.email}</span>
                  </div>
                  <button 
                    onClick={() => {
                      navigator.clipboard.writeText(reportData.email);
                      triggerToast(lang === "fa" ? "ایمیل کپی شد" : "Email address copied", "success");
                    }}
                    className="p-2 bg-slate-500/10 hover:bg-slate-500/20 rounded-xl text-slate-350 hover:text-white transition-all cursor-pointer shrink-0"
                    title="Copy Email"
                  >
                    <Copy className="w-4 h-4" />
                  </button>
                ...
                <div className="p-3 bg-slate-500/5 rounded-2xl border border-slate-500/10 flex items-center justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <span className="text-xs font-mono font-medium block mt-1 break-all leading-tight text-amber-300">{reportData.global_key || globalKey || '-'}</span>
                  </div>
                  <button 
                    onClick={() => {
                      const key = reportData.global_key || globalKey || '';
                      navigator.clipboard.writeText(key);
                      triggerToast(lang === "fa" ? "کلید API کپی شد" : "Global API key copied", "success");
                    }}
                    className="p-2 bg-slate-500/10 hover:bg-slate-500/20 rounded-xl text-slate-350 hover:text-white transition-all cursor-pointer shrink-0"
                ...
                <div className="p-3 bg-slate-500/5 rounded-2xl border border-slate-500/10 flex items-center justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <span className="text-xs font-mono font-medium block mt-1 break-all leading-tight text-slate-300">{reportData.kv}</span>
                  </div>
                  <button 
                    onClick={() => {
                      navigator.clipboard.writeText(reportData.kv);
                      triggerToast(lang === "fa" ? "نام KV کپی شد" : "KV Namespace name copied", "success");
                    }}
                    className="p-2 bg-slate-500/10 hover:bg-slate-500/20 rounded-xl text-slate-350 hover:text-white transition-all cursor-pointer shrink-0"
                    title="Copy KV Namespace"
                  >
                    <Copy className="w-4 h-4" />
                  </button>
                </div>

                {/* D1 Relational Database Name */}
                <div className="p-3 bg-slate-500/5 rounded-2xl border border-slate-500/10 flex items-center justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <span className="text-xs font-mono font-medium block mt-1 break-all leading-tight text-slate-300">{reportData.db}</span>
                  </div>
                  <button 
                    onClick={() => {
                      navigator.clipboard.writeText(reportData.db);
                      triggerToast(lang === "fa" ? "نام پایگاه داده D1 کپی شد" : "D1 Database UUID copied", "success");
                    }}
                    className="p-2 bg-slate-500/10 hover:bg-slate-500/20 rounded-xl text-slate-350 hover:text-white transition-all cursor-pointer shrink-0"
                    title="Copy D1 DB Name"
                  >
                    <Copy className="w-4 h-4" />
                  </button>
                </div>
        ...
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button 
              ...
              className={`p-2 bg-slate-500/10 text-indigo-400 border border-indigo-500/20 rounded-xl text-xs flex items-center gap-1.5 font-bold transition-all ${
                isDeploying ? "opacity-40 cursor-not-allowed" : "hover:bg-slate-500/20 cursor-pointer"
              }`}
            >
            ...
            <button
              ...
              className={`px-4 py-2 text-xs font-semibold text-indigo-400 bg-indigo-500/10 rounded-xl border border-indigo-500/20 transition-all flex items-center gap-1.5 ${
                isDeploying ? "opacity-40 cursor-not-allowed" : "hover:bg-indigo-500/20 cursor-pointer"
              }`}
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 md:p-6 grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-5 flex justify-center">
          ...
          <div className={`relative w-full max-w-[390px] min-h-[730px] rounded-[44px] shadow-2xl border-[11px] overflow-hidden flex flex-col transition-all ${
            ...
            <div className={`px-5 flex gap-4 border-b border-slate-500/10 mt-2 ${isDeploying ? "opacity-55 pointer-events-none" : ""}`}>
              <button 
                id="phone_tab_deploy"
              ...
              >
                ...
                {deployedAssets.length > 0 && (
                  <span className="absolute -top-1 -right-3.5 bg-indigo-600 text-white text-[8px] px-1.5 py-0.5 rounded-full font-bold">
                    {deployedAssets.length}
                  </span>
                )}
              </button>
              <button 
                id="phone_tab_options"
            ...
            <div className={`flex-1 p-5 overflow-y-auto space-y-4 ${
              ...
              {activeTab === "deploy" ? (
                ...
                <form id="phone_deploy_form" onSubmit={handleDeploy} className="space-y-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <input 
                      ...
                      className={`w-full h-11 border rounded-xl px-3 text-xs focus:outline-hidden focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/20 transition-all font-medium disabled:opacity-60 disabled:cursor-not-allowed ${
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="relative">
                      <input 
                        ...
                        className={`w-full h-11 border rounded-xl pl-3 pr-10 text-xs focus:outline-hidden focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/20 transition-all font-mono disabled:opacity-60 disabled:cursor-not-allowed ${
                          isDarkMode ? "bg-[#11192e] border-slate-800 text-white placeholder-slate-500" : "bg-white border-slate-200 text-slate-900 placeholder-slate-400"
                        }`}
                      />
                  ...
                  <div className="space-y-1">
                    ...
                    <input 
                      ...
                      className={`w-full h-11 border rounded-xl px-3 text-xs focus:outline-hidden focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/20 transition-all font-mono disabled:opacity-60 disabled:cursor-not-allowed ${
                        isDarkMode ? "bg-[#11192e] border-slate-800 text-white placeholder-slate-500" : "bg-white border-slate-200 text-slate-900 placeholder-slate-400"
                      }`}
                    />
                  </div>
                    ...
                    <div className="col-span-12 sm:col-span-5 space-y-1">
                      <label className="text-[10px] font-bold text-slate-400 uppercase flex items-center justify-between">
                        ...
                        <button
                          ...
                          className="text-slate-450 hover:text-indigo-400 cursor-pointer disabled:opacity-40"
                        >
                  ...
                  {/* Cancel/Stop button available during active deployment */}
                  {isDeploying && (
                    <button 
                      type="button"
                      onClick={handleCancel}
                      className="w-full h-11 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 border border-rose-500/15 font-bold text-xs rounded-2xl transition-all flex items-center justify-center gap-2 select-none cursor-pointer"
                    >
                      <XCircle className="w-4 h-4" />
                      <span>{lang === "fa" ? "لغو فرایند نصب (Cancel)" : "Cancel Deployment"}</span>
                    </button>
                  )}
                ...
                /* Options tab inside smartphone frame */
                <div className="space-y-4 font-sans text-xs animate-fade-in">
                  <div className="border-b pb-2.5 border-slate-500/10 mb-3">
                    <h4 className="font-bold text-slate-200 text-sm flex items-center gap-1.5">
                      <Server className="w-4 h-4 text-indigo-400 animate-pulse" />
                      {t.optTitle}
                    </h4>
                    <p className="text-[10.5px] text-slate-400 mt-1">{t.optDesc}</p>
                  </div>

                  {/* 1. Startup language chooser */}
                  <div className="space-y-1.5">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        type="button"
                        onClick={() => {
                          setFletDefaultLanguage("fa");
                          triggerToast(t.optSavedToast, "success");
                        }}
                        className={`h-9 rounded-xl font-bold cursor-pointer transition text-[11px] ${
                          fletDefaultLanguage === "fa" 
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          setFletDefaultLanguage("en");
                          triggerToast(t.optSavedToast, "success");
                        }}
                        className={`h-9 rounded-xl font-bold cursor-pointer transition text-[11px] ${
                          fletDefaultLanguage === "en" 
                            ? "bg-indigo-600 text-white shadow-md shadow-indigo-950/20" 
                            : "bg-slate-500/10 text-slate-400 hover:bg-slate-500/20"
                        }`}
                      >
                  ...
                  {/* 2. Primary Seed Theme Accent chooser */}
                  <div className="space-y-1.5 pt-2">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">
                      {t.optThemeAccent}
                    </label>
                    <div className="flex flex-wrap gap-2">
                      {(["indigo", "emerald", "amber", "crimson", "teal"] as const).map((clr) => {
                        const clrBg = 
                          clr === "indigo" ? "bg-indigo-500" :
                          clr === "emerald" ? "bg-emerald-500" :
                          clr === "amber" ? "bg-amber-500" :
                          clr === "crimson" ? "bg-rose-500" : "bg-teal-500";
                        return (
                          <button
                            key={clr}
                            type="button"
                            onClick={() => {
                              setFletThemeAccent(clr);
                              triggerToast(t.optSavedToast, "success");
                            }}
                            className={`px-3 py-1.5 rounded-lg text-[10px] font-extrabold capitalize flex items-center gap-1.5 cursor-pointer transition ${
                              fletThemeAccent === clr
                                ? "bg-indigo-600 text-white border border-indigo-400 shadow-md"
                                : "bg-slate-500/10 text-slate-350 hover:bg-slate-500/20"
                            }`}
                          >
                            <span className={`w-2.5 h-2.5 rounded-full ${clrBg} inline-block`}></span>
                            {clr}
                          </button>
                        );
                      })}
                  ...
                  {/* 3. Automatic Loader Toggle switch */}
                  <div className="p-3 bg-slate-500/5 rounded-xl border border-slate-500/10 space-y-2 mt-4">
                    <div className="flex items-center justify-between">
                      <span className="font-bold text-slate-300 text-[11px]">{t.optAutoInstall}</span>
                      <button
                        type="button"
                        onClick={() => {
                          setFletSelfInstall(!fletSelfInstall);
                          triggerToast(t.optSavedToast, "success");
                        }}
                        className={`w-10 h-5.5 rounded-full transition-all p-0.5 relative cursor-pointer ${
                          fletSelfInstall ? "bg-emerald-500" : "bg-slate-700"
                        }`}
                      >
                        <div className={`w-4.5 h-4.5 bg-white rounded-full shadow-md transition-all transform ${
                          fletSelfInstall ? "translate-x-4.5" : "translate-x-0"
                        }`}></div>
                      </button>
                    </div>
                    <p className="text-[9.5px] text-slate-400 leading-relaxed">
                      {t.optAutoInstallDesc}
                    </p>
                  </div>
            ...
            <div className={`border-t flex flex-col items-center py-2 relative z-20 ${isDeploying ? "opacity-55 pointer-events-none" : ""} ${
              ...
              <div className="w-full h-11 flex justify-around items-center px-6">
                ...
                <button 
                  onClick={() => setActiveTab("options")}
                  className={`flex flex-col items-center gap-1 transition-all cursor-pointer ${
                    activeTab === "options" ? "text-indigo-400 opacity-100" : "text-slate-400 opacity-60"
                  }`}
                >
                  <div className={`w-1.5 h-1.5 rounded-full ${activeTab === "options" ? "bg-indigo-400" : "bg-transparent"}`}></div>
                  <Server className="w-4 h-4" />
                  <span className="text-[8px] font-bold">{t.tab_options}</span>
                </button>
          ...
          <div className={`border p-4 rounded-2xl space-y-3 ${
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5 text-xs leading-normal">
              ...
              <div className={`p-3 rounded-xl border ${isDarkMode ? "bg-[#11192e] border-slate-800":"bg-white border-slate-200"}`}>
                ...
                <code className="block bg-slate-950 text-slate-350 font-mono text-[10px] p-2.5 rounded-lg mt-2 border border-slate-850 whitespace-pre-wrap leading-relaxed">
                  {lang === "fa" 
                    ? "flet run main.py\n\n# اگر با ارور مواجه شدید از دستور جایگزین زیر استفاده کنید:\npython -m flet run main.py" 
                    : "flet run main.py\n\n# If command not found, use python wrapper alternative:\npython -m flet run main.py"}
                </code>
              </div>
